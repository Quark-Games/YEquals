...
def cg(c, e) -> tuple:
    ...
    gap_py = int(e[1])
    exp = e[2]
    tl = evaluate(x, y, exp[0], e[3:7]) - evaluate(x, y, exp[1], e[3:7])
    tr = evaluate(x + gap_px, y, exp[0], e[3:7]) - evaluate(x + gap_px, y, exp[1], e[3:7])
    bl = evaluate(x, y + gap_py, exp[0], e[3:7]) - evaluate(x, y + gap_py, exp[1], e[3:7])
    br = evaluate(x + gap_px, y + gap_py, exp[0]) - evaluate(x + gap_px, y + gap_py, exp[1], e[3:7])
    temp = int(bool(tl > 0) << 3 | bool(tr > 0) << 2 | bool(bl > 0) << 1 | bool(br > 0))
    if temp in (0b0000, 0b0110, 0b1001, 0b1111):
        return None
    output = []
    if sgn(tl) != sgn(tr):
        t = abs(tl)/(abs(tl) + abs(tr))
        output.append((x + int(gap_px * t), y))
    if sgn(bl) != sgn(br):
        t = abs(bl)/(abs(bl) + abs(br))
        output.append((x + int(gap_px * t), y + gap_py))
    if sgn(tl) != sgn(bl):
        t = abs(tl)/(abs(tl) + abs(bl))
        output.append((x, y + int(gap_py * t)))
    if sgn(tr) != sgn(br):
        t = abs(tr)/(abs(tr) + abs(br))
        output.append((x + gap_px, y + int(gap_py * t)))
    return tuple(output)
    ...
    class Relation(Func):
        ...
        def graph(self, exp, domain="True"):
            ...
            try:
                ...
                # compute
                points = multiprocessing.Pool().starmap(
                    cg,
                    zip(
                        itertools.product(
                        ...
                        itertools.repeat(
                            (gap_px, gap_py, r, ori_x, ori_y, coor.scalex, coor.scaley),
                        ),
                    )
                )
